...
usage()
{
  ...
  echo ""
  echo "usage: $0 [--verbose | -v] [--help | -h] [(--olm_version | -o) <version_number>] [--skip_olm_installation | -s] [(--olm_global_catalog_namespace | -c) <OLM global catalog namespace> ]"
  echo "options:"
...
#      TRANSIENT_FAILURE can switch to READY state
#      if [ "$cs_state" == "TRANSIENT_FAILURE" ]; then
#        echo "Error: Problem during installation of Catalog Source, check README for manual installation and troubleshooting"
#        exit 12
#      fi
    fi
    sleep 1
