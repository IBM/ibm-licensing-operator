...
show_url(){
  if ! route_url=$(quiet_err_command kubectl get route ibm-licensing-service-instance -o jsonpath='{.status.ingress[0].host}' -n "$INSTALL_NAMESPACE") || [ "${route_url}" == "" ]; then
    verbose_output_command log "Could not get Route for License Service in $INSTALL_NAMESPACE, Route CRD might not be available at your cluster, or gateway option was chosen"
  else
