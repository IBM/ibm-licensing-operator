...
func UpdateCacheClusterExtensions(client c.Reader, logger logr.Logger) error {
	...
	// Check for Gateway API CRDs without namespace restriction to detect if CRD exists
	if err := client.List(context.TODO(), gatewayTestInstance); err == nil {
		IsGatewayAPI = true
		logger.Info("Gateway API CRDs detected in cluster")
	} else {
		IsGatewayAPI = false
		logger.Info("Gateway API CRDs not found in cluster. Gateway routing features will be disabled. To enable Gateway support, install Gateway API CRDs see documentation about needed cluster extensions ibm.biz/LS_gateway_API", "error", err.Error())
	}

	backendTLSPolicyTestInstance := &gatewayv1.BackendTLSPolicyList{}
	if err := client.List(context.TODO(), backendTLSPolicyTestInstance); err == nil {
