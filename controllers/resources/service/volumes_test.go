...
func TestGetLicensingVolumesEnabled(t *testing.T) {
	...
	assert.Equal(t, spec.GetDefaultReporterTokenSecret(), reporterTokenVolume.Secret.SecretName, "Sender reporter token secret not specified, volume should have default name.")

	spec.Sender.ReporterTokenSecret = "someSecretName"
	volumes = getLicensingVolumes(spec)
	reporterTokenVolume = volumes[3]
