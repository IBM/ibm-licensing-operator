...
prepare-unit-test:
	...
	@if [ -e ./artifactory.yaml ]; then \
		...
		kubectl create secret docker-registry artifactory-token -n ${NAMESPACE} --docker-server=${REGISTRY} --docker-username=${ARTIFACTORY_USER} --docker-password=${ARTIFACTORY_TOKEN} || echo "" ;\
	fi ;\
